...
## 8-1. 요구사항
1. 화면(Scene) 구성  
   1-1. 시작화면, 게임화면 총 2개의 화면이 있다.
   
2. 시작화면 구성  
   2-1. 시작화면에는 시작과 옵션, 게임종료 버튼이 각각 있다.  
   2-2. 게임종료 버튼을 누르면 게임이 종료된다.  
   2-3. 옵션 버튼을 누르면 옵션 UI창이 나타난다.  
   2-4. 옵션 창에서는 사운드를 조절할 수 있다.  
   2-5. 옵션 창의 우측 하단에 시작화면으로 돌아갈 수 있는 버튼(Title)이 있다.  
   2-6. 시작화면에서 시작 버튼을 누르면 게임화면으로 이동한다.
   
3. 게임화면 구성  
   3-1. 게임화면에는 하단에 텍스트창과 우측 하단에 시스템 버튼들이 존재한다.  
   3-2. 화면 전체 크기의 이미지 오브젝트가 있다. 이는 배경이미지가 들어간다.  
   3-3. 화면의 중앙 좌측부터 일정한 간격으로 이미지 오브젝트가 5개 있다. 이는 캐릭터 이미지가 들어간다.  
  
4. 플레이어 조작 구성  
   4-1. 마우스 좌클릭으로 텍스트를 넘기며 진행이 가능하다.  
   4-2. 마우스 휠을 위로 올려 이전 텍스트를 볼 수 있는 UI를 띄울 수 있다.  
   4-3. 마우스 우클릭으로 옵션화면을 띄울 수 있다.  
   4-4. 왼쪽 ctrl 키로 텍스트를 스킵할 수 있다.  
   
6. 시스템 버튼 구성  
   5-1. 세이브를 할 수 있는 세이브 버튼이 있다.  
   5-2. 세이브한 파일을 불러올 수 있는 로드 버튼이 있다.  
   5-3. 시작화면과 동일한 옵션화면을 띄울 수 있는 옵션 버튼이 있다.  
   5-4. 텍스트를 스킵할 수 있는 스킵 버튼이 있다.  
   5-5. 텍스트를 일정시간(텍스트 길이에 따라 다름)이 지나면 자동으로 넘겨주는 오토 버튼이 있다.
