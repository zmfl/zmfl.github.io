...
3. 게임화면 구성  
   ...
   3-4. 마우스 좌클릭으로 텍스트를 넘기며 진행이 가능하다.  
   3-5. 마우스 휠을 위로 올려 이전 텍스트를 볼 수 있는 UI를 띄울 수 있다.  
   3-6. 왼쪽 ctrl 키로 텍스트를 스킵할 수 있다.  
   
4. 시스템 버튼 구성  
   4-1. 세이브를 할 수 있는 세이브 버튼이 있다.  
   4-2. 세이브한 파일을 불러올 수 있는 로드 버튼이 있다.  
   4-3. 시작화면과 동일한 옵션화면을 띄울 수 있는 옵션 버튼이 있다.  
   4-4. 텍스트를 스킵할 수 있는 스킵 버튼이 있다.  
   4-5. 텍스트를 일정시간(텍스트 길이에 따라 다름)이 지나면 자동으로 넘겨주는 오토 버튼이 있다.
